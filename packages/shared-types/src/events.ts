import { QueueItem, RoomMember, RoomState } from './models';
...
export interface AddQueueItemPayload {
  roomCode: string;
  videoId: string;
  title: string;
  thumbnailUrl: string;
...
export interface RemoveQueueItemPayload {
  roomCode: string;
  queueItemId: string;
  hostToken?: string;
  nickname?: string;
...
export interface PlayerSkipPayload {
  roomCode: string;
  hostToken?: string;
}
...
export interface PlayerTimeUpdatePayload {
  roomCode: string;
  currentTime: number;
  duration: number;
}
